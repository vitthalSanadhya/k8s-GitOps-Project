...
# Defined at root level (not in IAM module) so that the
# oidc_provider_arn / url are known concrete values, not
# unknown-at-plan-time expressions — avoids the
# "count depends on resource attributes not yet known" error.
###############################################################
